...
type GameMode = "solar-system" | "braver-angels";

const queryParameters = new URLSearchParams(window.location.search);

let gameMode: GameMode = "solar-system";

if (queryParameters.get("mode") === "braver-angels") {
...
const ChatWindow: FunctionComponent<ChatWindowProps> = ({
  ...
  const initialMessage = useMemo(() => {
    return getInitialMessage(gameMode);
  }, []);
  const systemMessage = useSystemMessage(tools, initialMessage, gameMode);
...
7. We believe that, in disagreements, both sides share and learn.
8. In Braver Angels, neither side is teaching the other or giving feedback on how to think or say things differently.
`;

const getInitialMessage = (gameMode: GameMode) => {
  if (gameMode === "solar-system") {
    return `
...
*This app is not affiliated with the Braver Angels organization.*

Are you ready to get started?`;
  } else {
    throw Error("Unexpected game mode.");
  }
};
...
Then you will judge the answer based on the below information.
...
Here's the information about the Braver Angels Way:

For the Braver Angels website:
